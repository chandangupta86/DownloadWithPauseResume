...
public class DownloadTask extends AsyncTask<Void, Integer, Long> {
    ...
    protected void onProgressUpdate(Integer... progress) {
        if (progress.length > 1) {
          ...
          /*  Log.e("test","downloadSize="+downloadSize);
            Log.e("test","downloadPercent="+downloadPercent);
            Log.e("test","networkSpeed="+networkSpeed);*/
            if (listener != null)
    ...
    private long download() throws NetworkErrorException, IOException {
        ...
        if (file.exists() && totalSize == file.length()) {
            if (DEBUG) {
                Log.v(null, "Output file already exists. Skipping download." + file.getAbsolutePath());
            }
